...
def mostrar_bienvenida(creadores=None, programa=None, universidad=None, semestre=None, profesor=None):
    """Muestra pantalla de bienvenida. Acepta datos opcionales del equipo: creadores, programa, universidad, semestre y profesor."""
    limpiar_pantalla()
    print("="*50)
    print("*" + " "*48 + "*")
    ...
    # Mostrar información detallada del equipo si está disponible
    if universidad or programa or semestre or profesor:
        if universidad:
            print(f"Universidad: {universidad}")
        if programa:
            print(f"Programa: {programa}")
        if semestre:
            print(f"Semestre del grupo: {semestre}")
        if profesor:
            print(f"Profesor: {profesor}")
        print()

    # Mostrar creadores si fueron proporcionados
    if creadores:
        print("Creadores:")
...
def eliminar_usuario_admin(gestor: auth.GestorUsuarios):
    ...
    while True:
        ...
        print("-"*60)
        for u in usuarios:
            marcador = " (admin)" if getattr(u, 'es_admin', False) else ""
            print(f" - {u.usuario}{marcador}")
        print("-"*60)

        usuario = input("Nombre de usuario a eliminar (o 'q' para volver): ").strip()
        if usuario.lower() in ('q', ''):
            break
        ...
        if confirmacion == "S":
            if gestor.eliminar_usuario(usuario):
                ...
                otra = input("¿Desea eliminar otro usuario? (S/N): ").upper()
                if otra == 'S':
                    continue
                else:
                    break
